// ==================== BUNKIT PWA SERVICE WORKER ====================

const CACHE_NAME = 'bunkit-v3.29';
...
self.addEventListener('fetch', (event) => {
    ...
    if (event.request.url.includes('supabase.co') ||
        ...
        event.request.url.includes('/presentation') ||
        event.request.url.includes('PRESENTATION.html') ||
    ...
    // === NETWORK-FIRST for core app files (always get latest code) ===
    const CORE_APP_FILES = ['/js/app.js', '/css/styles.css'];
    const isCoreAppFile = CORE_APP_FILES.some(f => event.request.url.includes(f));
...
// === PUSH EVENT HANDLER (Web Push API / VAPID) ===
// This handles incoming push messages from our Supabase Edge Function
self.addEventListener('push', (event) => {
    ...
    // Parse VAPID push payload from Edge Function
    if (event.data) {
        try {
    ...
    event.waitUntil(
        (async () => {
            ...
            if (appClients.length > 0) {
                ...
                if (targetUrl.includes('openLog=true')) {
                    client.postMessage({ type: 'OPEN_DAILY_LOG' });
                    console.log('SW: Focused window and sent OPEN_DAILY_LOG');
                } else if (targetUrl !== '/') {
                    await client.navigate(targetUrl);
                    console.log('SW: Navigated to ' + targetUrl);
                }
            } else {
                // No existing window, open a new one
                await clients.openWindow(targetUrl);
                console.log('SW: Opened new window at ' + targetUrl);
            }
        })()
    );
});
