...
const SUPABASE_URL = 'https://rqajrgqpfuqfvwveyqdh.supabase.co'; // original direct URL
...
// 🔧 Jio Block Fix: Use Vercel reverse proxy instead of direct supabase.co
// Routes all Supabase traffic through YOUR Vercel domain to bypass ISP censorship
const SUPABASE_PROXY_URL = window.location.hostname === 'localhost'
    ? 'https://rqajrgqpfuqfvwveyqdh.supabase.co'      // Direct URL for local dev
    : window.location.origin + '/api/supabase-proxy';  // Vercel proxy for production
...
if (typeof supabase !== 'undefined') {
    if (SUPABASE_URL === 'YOUR_SUPABASE_URL' || SUPABASE_ANON_KEY === 'YOUR_SUPABASE_ANON_KEY') {
        ...
        try {
            // Use proxy URL in production to bypass ISP (Jio) blocks
            supabaseClient = supabase.createClient(SUPABASE_PROXY_URL, SUPABASE_ANON_KEY);
            console.log('✅ Supabase Client Initialized via:', SUPABASE_PROXY_URL);
        } catch (e) {
