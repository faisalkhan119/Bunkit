...
const CommunityManager = {
            ...
            alertBox.innerHTML = `
                <div>
                    <div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
                        ...
                        <div>
                            <strong>Polls Enabled (ID: ${this.currentClassId.substring(0, 8)})</strong>
                            <div style="font-size:0.82rem; opacity:0.8; margin-top:2px;">${checkedInCount}/${totalMembers} checked in, ${readyCount} ready. Polls are open, but review risks below.</div>
                        </div>
                    </div>
                    <div style="background:rgba(0,0,0,0.03); border-radius:8px; padding:10px 12px; margin-top:8px;">
                        <div style="font-size:0.8rem; font-weight:600; color:#b25810; margin-bottom:6px;">❓ Warnings:</div>
                        ${reasonHTML}
                    </div>
                </div>`;
    ...
    async createPoll(subject, targetDate, message) {
        // Relaxed criteria: Users can always create polls now.
        try {
