                    ...
                    <div id="profileNameModal" class="modal">
                        <div class="modal-content" style="max-width: 400px;">
                            ...
                            <div class="modal-header">
                                <h2>✏️ Account Center</h2>
                                <p>View your subscription & settings</p>
                            </div>
                            
                            <!-- PREMIUM STATUS BADGE -->
                            <div id="premiumStatusBadge" style="margin-bottom: 20px; padding: 12px; background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; text-align: center; display: none;">
                                <div style="font-size: 0.85rem; color: #8896ab; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px;">Current Plan</div>
                                <div id="premiumStatusText" style="font-weight: 700; font-size: 1.1rem; color: #f1f5f9; display: flex; align-items: center; justify-content: center; gap: 6px;">
                                    <span id="premiumStatusIcon"></span>
                                    <span id="premiumStatusName">Loading...</span>
                                </div>
                                <div id="premiumStatusExpiry" style="font-size: 0.8rem; color: #8896ab; margin-top: 4px;"></div>
                            </div>
                            <div class="form-group" style="margin-bottom: 15px;">
                                <label style="font-size: 0.85rem; color: #8896ab;">Email Address</label>
                                <input type="email" id="profileEmailInput" disabled style="background: rgba(255,255,255,0.02); color: #cbd5e1; cursor: not-allowed; border: 1px dashed rgba(255,255,255,0.1);">
                            </div>

                            <div class="form-group" style="margin-bottom: 15px;">
                                <label style="font-size: 0.85rem; color: #8896ab;">Display Name</label>
                                <input type="text" id="profileNameInput" placeholder="Enter your name" maxlength="50">
                            </div>

                            <div class="form-group" style="margin-bottom: 25px;">
                                <label style="font-size: 0.85rem; color: #8896ab;">Phone Number</label>
                                <input type="tel" id="profilePhoneInput" placeholder="+91 XXXXX XXXXX">
                            </div>

                            <div style="border-top: 1px solid rgba(255,255,255,0.1); margin: 20px 0; padding-top: 20px;">
                                <h3 style="font-size: 1rem; margin-bottom: 15px;">Security</h3>
                                <div class="form-group" style="margin-bottom: 15px;">
                                    <label style="font-size: 0.85rem; color: #8896ab;">Current Password</label>
                                    <input type="password" id="profileOldPasswordInput" placeholder="Enter current password" minlength="6">
                                </div>
                                <div class="form-group" style="margin-bottom: 15px;">
                                    <label style="font-size: 0.85rem; color: #8896ab;">New Password</label>
                                    <input type="password" id="profileNewPasswordInput" placeholder="Enter new password" minlength="6">
                                </div>
                                <div class="form-group" style="margin-bottom: 15px;">
                                    <label style="font-size: 0.85rem; color: #8896ab;">Confirm New Password</label>
                                    <input type="password" id="profileConfirmPasswordInput" placeholder="Confirm new password" minlength="6">
                                </div>
                            </div>

                            <div class="form-actions" style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 20px;">
                                <button class="btn secondary-btn" onclick="closeModal('profileNameModal')">Cancel</button>
                                <button class="btn primary-btn" onclick="saveProfileName()" id="saveProfileBtn">💾 Save Changes</button>
                            </div>
                        </div>
                    </div>`;
...
async function openProfileNameModal() {
    ...
    const oldPassInput = document.getElementById('profileOldPasswordInput');
    ...
    if (emailInput) emailInput.value = window.AuthManager?.user?.email || 'Guest User';

    // Fetch user profile from Supabase to get phone number
    if (phoneInput && window.AuthManager?.user && window.supabaseClient) {
        try {
            const { data } = await window.supabaseClient.from('profiles').select('phone').eq('id', window.AuthManager.user.id).single();
            phoneInput.value = data?.phone || '';
        } catch (e) {
            console.warn('Failed to fetch phone', e);
        }
    }

    // --- PREMIUM STATUS LOGIC ---
    const badge = document.getElementById('premiumStatusBadge');
    const statusName = document.getElementById('premiumStatusName');
    const statusIcon = document.getElementById('premiumStatusIcon');
    const statusExpiry = document.getElementById('premiumStatusExpiry');

    if (badge && statusName && window.AuthManager?.user && window.supabaseClient) {
        badge.style.display = 'block';
        statusName.textContent = 'Checking Status...';
        ...
        try {
            const { data, error } = await window.supabaseClient
                .from('app_config')
                .select('value')
                .eq('key', 'premium_users')
                .single();

            const userEmail = window.AuthManager.user.email.trim().toLowerCase();
            let hasPremium = false;

            if (data?.value && data.value[userEmail]) {
                const planData = data.value[userEmail];
                if (Date.now() <= planData.expires_at) {
                    hasPremium = true;
                    statusIcon.textContent = '👑';
                    statusIcon.style.color = '#fbbf24'; // Amber
                    ...
                    statusName.textContent = plans[planData.plan] || 'Premium Member';
                    statusName.style.color = '#fbbf24';

                    if (planData.plan === 'lifetime') {
                        statusExpiry.textContent = 'Never Expires';
                    } else {
                        const expiryDate = new Date(planData.expires_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' });
                        statusExpiry.textContent = `Expires on ${expiryDate}`;
                    }

                    badge.style.background = 'rgba(251, 191, 36, 0.05)';
            ...
            if (!hasPremium) {
                statusIcon.textContent = '🌱';
                statusIcon.style.removeProperty('color');
                statusName.textContent = 'Free Tier';
                statusName.style.removeProperty('color');
                statusExpiry.textContent = 'Ad-supported experience';
            ...
            statusName.textContent = 'Free Tier';
            statusIcon.textContent = '🌱';
            statusExpiry.textContent = 'Unable to verify subscription';
        }
    } else if (badge) {
        // Guest mode
        badge.style.display = 'block';
        statusIcon.textContent = '👤';
        statusName.textContent = 'Guest Tier';
        statusExpiry.textContent = 'Sign in to unlock premium';
    }
    // ----------------------------
...
async function saveProfileName() {
    ...
    try {
        ...
        // 1. Save local name & sync
        localStorage.setItem('userProfileName', name);
        updateSidebarAccountUI();
        if (window.SyncManager) SyncManager.saveSettings();

        // 2. Authed Updates (Phone & Password)
        if (window.AuthManager?.user && window.supabaseClient) {
            // Update Phone in Profiles
            try {
                const { error: phoneErr } = await window.supabaseClient
                    .from('profiles')
                    .update({ phone: phone || null })
                    .eq('id', window.AuthManager.user.id);
                if (phoneErr) console.warn('Failed to update phone:', phoneErr);
            } catch (e) { console.warn('Phone update threw error', e); }

            // Validate and trigger Password Change if filled
            const anyPassFilled = oldPass.length > 0 || newPass.length > 0 || confirmPass.length > 0;
            if (anyPassFilled) {
                if (!oldPass || !newPass || !confirmPass) {
                    alert('⚠️ Please fill out all three password fields to change your password.');
                    if (saveBtn) saveBtn.textContent = '💾 Save Changes';
                    return;
                }
                ...
                if (typeof window.AuthManager.updatePassword === 'function') {
                    const passSuccess = await window.AuthManager.updatePassword(oldPass, newPass);
                    if (!passSuccess) {
                        if (saveBtn) saveBtn.textContent = '💾 Save Changes';
                        return; // Stop if password update failed (it handles its own alert)
                    }
                }
            }
        }
        ...
        alert('⚠️ An unexpected error occurred while saving.');
    } finally {
        if (saveBtn) saveBtn.textContent = '💾 Save Changes';
    }
}
...
function checkForClassImport() {
    ...
    const sharedClassId = urlParams.get('shared_class_id');

    if (sharedClassId) {
        // Store for import after onboarding completes
        sessionStorage.setItem('pendingSharedClassId', sharedClassId);

        // Clean URL without reload to avoid messy URL
        const cleanUrl = window.location.origin + window.location.pathname;
        window.history.replaceState({}, document.title, cleanUrl);

        console.log("🔗 Shared class ID detected and stored.");
    } else if (classData) {
        // Store for import after onboarding completes
...
async function importSharedClassFromDB(sharedId) {
    console.log(`🔗 Fetching Shared Class [${sharedId}] from DB...`);
    try {
        // FIX 2: Wait for supabaseClient to be ready (up to 3 seconds)
        ...
        if (!window.supabaseClient) {
            throw new Error("Cloud sync is not connected. Please check your internet and try again.");
        }

        // Quick check: Does the user already have this exact shared class imported?
        const existingClassNames = Object.keys(classes);
        for (let name of existingClassNames) {
            if (classes[name].sharedId === sharedId) {
                console.log(`Class with sharedId ${sharedId} already exists locally as ${name}.`);
                alert(`⚠️ You already have this class ("${name}") in your Bunkit!`);
                sessionStorage.removeItem('pendingSharedClassId');
        ...
        // FIX 3: Better error messages for table-not-found and RLS errors
        if (error) {
            console.error("Fetch Shared Class Error:", error);
            const code = error.code || '';
            const msg = (error.message || '').toLowerCase();
            if (code === '42P01' || msg.includes('relation') || msg.includes('does not exist')) {
                throw new Error("Server setup incomplete. The shared_classes table does not exist yet. Please ask the class owner to update their app.");
            } else if (code === 'PGRST116' || msg.includes('rows returned')) {
                throw new Error("Class not found. The link may be invalid or the class was deleted.");
            } else {
                throw new Error("Class not found or link has expired.");
            }
        }
        if (!data || !data.class_data) {
            throw new Error("Class not found or link has expired.");
        }
        ...
        const cleanClassData = {
            ...
            timetable: classData.timetable || classData.timetableArrangement || {},
        ...
        sessionStorage.removeItem('pendingSharedClassId');
        importClassFromURL(encodedData);

    } catch (error) {
        console.error('Failed to fetch shared class:', error);
        sessionStorage.removeItem('pendingSharedClassId');

        let errorMessage = error.message;
        if (errorMessage.includes('not connected') || errorMessage.includes('offline')) {
            errorMessage = "It looks like you're offline or Bunkit can't reach the sync server. Please check your internet connection and try again.";
        } else if (errorMessage.includes('not found') || errorMessage.includes('expired') || errorMessage.includes('deleted')) {
            // Keep the specific message from the throw above
        } else if (errorMessage.includes('Server setup')) {
            // Keep the specific message from the throw above
        }

        const safeError = errorMessage.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
        alert(`❌ Failed to import shared class.\n\n${safeError}`);
    }
}

function importClassFromURL(encodedData) {
    if (!encodedData || encodedData === 'null') {
        console.warn("🔗 No import data provided to importClassFromURL.");
        return;
    }
    ...
    try {
        ...
        // If shared class had a startDate, set it in portalSetup
        if (classData.startDate && !classes[finalName].portalSetup?.semesterStartDate) {
            if (!classes[finalName].portalSetup) classes[finalName].portalSetup = {};
            classes[finalName].portalSetup.semesterStartDate = classData.startDate;
        }
...
function saveFirstLoginName() {
    ...
    if (name) {
        ...
        const pendingSharedClass = sessionStorage.getItem('pendingSharedClassId');
        const pendingClass = sessionStorage.getItem('pendingClassImport');
        if (pendingSharedClass) {
            // Skip class choice modal and import directly
            setTimeout(() => importSharedClassFromDB(pendingSharedClass), 350);
...
// FIX 1: Handle Existing Users who click the link — wait for DOM + SDK to be fully ready
document.addEventListener('DOMContentLoaded', () => {
    if (localStorage.getItem('hasCompletedOnboarding') === 'true') {
        const pendingSharedClass = sessionStorage.getItem('pendingSharedClassId');
        const pendingClass = sessionStorage.getItem('pendingClassImport');
        if (pendingSharedClass) {
            // Delay to ensure Supabase SDK is loaded (importSharedClassFromDB also has its own retry)
            setTimeout(() => importSharedClassFromDB(pendingSharedClass), 1000);
        } else if (pendingClass) {
...
function checkFirstLoginPrompt() {
    ...
    const pendingSharedImport = sessionStorage.getItem('pendingSharedClassId');
    const pendingImport = sessionStorage.getItem('pendingClassImport');
    if (pendingSharedImport || pendingImport) {
        ...
        if (!profileName) {
            ...
            if (pendingSharedImport) {
                setTimeout(() => importSharedClassFromDB(pendingSharedImport), 500);
            } else {
    ...
    modal.innerHTML = `
                <div class="modal-content" style="max-width: 400px;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 15px; padding: 15px 0;">
                        <button class="btn primary-btn" onclick="openOnboardingChoice('new');" style="padding: 18px; font-size: 1.1rem;">
                            ➕ Create New Class
                        </button>
                        <button class="btn secondary-btn" onclick="openOnboardingChoice('example');" style="padding: 18px; font-size: 1.1rem;">
                            📝 Use Example Class
                        </button>
                    </div>
                </div>`;
...
window.updateSidebarAccountUI = function () {
    ...
    const premiumTag = document.getElementById('sidebarPremiumTag');
    ...
    if (isSignedIn) {
        ...
        if (premiumTag) {
            premiumTag.style.display = 'none'; // Default to hidden
            if (window.supabaseClient && authManagerUser?.email) {
                (async () => {
                    try {
                        ...
                        const email = authManagerUser.email.trim().toLowerCase();
                        if (data?.value && data.value[email]) {
                            const planData = data.value[email];
                            if (Date.now() <= planData.expires_at) {
                                premiumTag.style.display = 'inline-block';
                            }
                        }
                    } catch (e) {
                        console.warn("Failed to check premium status for sidebar", e);
                    }
                })();
            }
        }
...
function initializeApp() {
    ...
    setTimeout(() => {
        try {
            ...
            window.onboardingChoiceModalHTML = `
                <div class="modal-content" style="max-width: 500px; text-align: center;">
                    <button class="modal-close" onclick="closeModal('onboardingChoiceModal')">&times;</button>
                    <div class="modal-header">
                        <div style="font-size: 3rem; margin-bottom: 10px;">✨</div>
                        <h2>Add Your Class</h2>
                    </div>
                    <div style="padding: 10px 0 20px 0;">
                        <p style="color: var(--medium-text); line-height: 1.6;">
                            Creating a class timetable can be tricky, even with AI. If you'd like, <strong>the Admin can do it for you</strong> in their free time!
                        </p>
                        <p style="font-size: 0.85rem; color: var(--primary-color); margin-top: 10px; font-weight: 500;">
                            (Recommended for 100% accuracy)
                        </p>
                    </div>
                    
                    <div style="display: flex; flex-direction: column; gap: 15px;">
                        <button class="btn primary-btn" onclick="triggerAdminSetupRequest()" style="padding: 18px; width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px;">
                            <span>💬</span> Ask Admin to Create (Free)
                        </button>
                        
                        <div style="display: flex; align-items: center; gap: 10px; margin: 5px 0;">
                            <hr style="flex: 1; border: 0; border-top: 1px solid var(--border-color);">
                            <span style="font-size: 0.8rem; color: var(--medium-text);">OR</span>
                            <hr style="flex: 1; border: 0; border-top: 1px solid var(--border-color);">
                        </div>
                        
                        <button class="btn secondary-btn" onclick="proceedWithOnboardingAction()" style="padding: 15px; width: 100%; border: 2px solid var(--border-color); background: transparent; color: var(--dark-text);">
                            🔨 I'll do it myself
                        </button>
                    </div>
                </div>`;
...
function getSubjectClassCountForDate(dateStr, subjectCode) {
    ...
    const normSearchCode = String(subjectCode || '').toUpperCase();
    ...
    if (customSchedule) {
        if (customSchedule._periods) {
            ...
            return customSchedule._periods.filter(code => String(code || '').toUpperCase() === normSearchCode).length;
        } else {
            // Old count-based custom schedule - keys might be in different case
            const normalizedCustom = {};
            Object.keys(customSchedule).forEach(k => { normalizedCustom[k.toUpperCase()] = customSchedule[k]; });
            return normalizedCustom[normSearchCode] || 0;
        }
    }
    ...
    if (dayArrangement.length > 0) {
        return dayArrangement.filter(item => {
            ...
            return String(code || '').toUpperCase() === normSearchCode;
        }).length;
    }

    // Fallback to default schedule
    const subject = selectedClass?.subjects?.find(s => String(s.code || '').toUpperCase() === normSearchCode);
    if (subject) {
...
async function saveNotificationSettings() {
    try {
        ...
        const time = '17:00'; // Global 5:00 PM schedule
        ...
        if (enabled) {
            ...
            // Integrate new VAPID Web Push Subscription here
            const userEmail = (AuthManager.user && AuthManager.user.email) ? AuthManager.user.email : null;
            const subSuccess = await pushManager.requestPermissionAndSubscribe(userEmail);
            if (!subSuccess) {
                showToast('Failed to subscribe to Web Push. Background notifications might not work.', 'error');
            }
        }
    ...
    modal.innerHTML = `
                <div class="modal-content" style="max-width: 450px; text-align: center;">
                    ...
                    <div style="display: grid; gap: 12px;">
                          <button class="btn primary-btn" onclick="closeModal('welcomeModal'); openOnboardingChoice()" style="padding: 15px;">
                            📝 <strong>Manual Entry</strong><br><span style="font-size:0.8rem; font-weight:normal;">Fill form manually</span>
                        </button>
                        
                        <button class="btn secondary-btn" onclick="closeModal('welcomeModal'); openOnboardingChoice();" style="padding: 15px; border: 2px solid var(--primary-color); color: var(--primary-color);">
                            ✨ <strong>Inbuilt AI Import</strong><br><span style="font-size:0.8rem; font-weight:normal;">AI reads your screenshots</span>
                        </button>

                         <button class="btn secondary-btn" onclick="closeModal('welcomeModal'); openOnboardingChoice();">
                             📸 <strong>Scan QR Code</strong><br><span style="font-size:0.8rem; font-weight:normal;">Import shared class</span>
                         </button>

                         <button class="btn secondary-btn" onclick="closeModal('welcomeModal'); openOnboardingChoice();">
                             📄 <strong>Import from JSON</strong><br><span style="font-size:0.8rem; font-weight:normal;">Paste data from AI prompt</span>
                         </button>
                    </div>
...
/**
 * Opens the onboarding choice modal
 */
function openOnboardingChoice(source = 'default') {
    window._onboardingSource = source;

    // Close existing onboarding modal if open
    closeModal('onboardingClassModal');

    const modal = document.getElementById('onboardingChoiceModal');
    if (modal && typeof onboardingChoiceModalHTML !== 'undefined') {
        modal.innerHTML = onboardingChoiceModalHTML;
    } else {
        console.warn('⚠️ onboardingChoiceModalHTML not loaded properly, falling back to direct action.');
        return proceedWithOnboardingAction();
    }
    openModal('onboardingChoiceModal');
}

/**
 * Handle "I'll do it myself" action based on source
 */
function proceedWithOnboardingAction() {
    const source = window._onboardingSource;
    closeModal('onboardingChoiceModal');
...
/**
 * Construct and trigger a WhatsApp message for Admin Assistance
 */
function triggerAdminSetupRequest() {
    const user = window.AuthManager?.user;
    const userEmail = user ? user.email : "Guest User";
    const phoneNumber = "916386854875";
    const source = window._onboardingSource;

    let extraNote = "";
    if (source === 'example') {
        extraNote = "\n(I can't edit the example class properly, please help!)";
    }

    const message = `Hi Admin! Please create my class timetable.${extraNote}\n\nMy Email: ${userEmail}\n\nHere are the details I need to provide you:\n- Timetable Screenshot/Image:\n- Last Working Date:\n- Semester Start Date (Optional):\n- Holidays List:\n\nThank you!`;
...
function openImportModal() { openOnboardingChoice(); setTimeout(() => switchModalTab('json'), 50); }
    ...
    modal.innerHTML = `
                <div class="modal-content" style="max-width: 500px; text-align: center;">
                    ...
                    <div class="modal-header" style="padding-bottom: 10px;">
                        <h2>👑 Unlock Ad-Free Premium</h2>
                        <p style="font-size: 0.95rem; color: var(--medium-text);">Support the developer & remove ads!</p>
                    </div>
                    ...
                    <div class="donate-message-box" style="background: var(--card-bg); padding: 20px; border-radius: 12px; margin-bottom: 20px; border: 2px solid var(--secondary-grad-start); box-shadow: 0 4px 15px rgba(79, 172, 254, 0.15);">
                        <p style="font-size: 1rem; color: var(--dark-text); margin-bottom: 15px; line-height: 1.6;">
                            Hey there! 👋 I'm a solo developer building this app to help students manage attendance. To keep the core app free, it runs on ads.
                        </p>
                        <p style="font-size: 0.95rem; color: var(--dark-text); margin-bottom: 15px; line-height: 1.6;">
                            Your support helps me unlock Premium for you:
                        </p>
                        <ul style="text-align: left; font-size: 0.9rem; color: var(--dark-text); padding-left: 25px; margin-bottom: 10px; line-height: 1.8;">
                            <li>🚫 Unlock the <strong>100% Ad-Free Premium tier</strong></li>
                            <li>💰 Cover AI/OCR Server Costs</li>
                            <li>☁️ Maintain cloud sync servers</li>
                            <li>✨ Help me build more features!</li>
                        </ul>
                    </div>
                    ...
                    <p style="font-size: 0.9rem; color: var(--medium-text); margin-top: 20px; font-style: italic;">
                        💖 Send me a WhatsApp message after donating to activate Premium!
                    </p>
    ...
    modal.innerHTML = `
                <div class="modal-content" style="max-width: 700px; max-height: 85vh; overflow-y: auto;">
                    ...
                    <div style="text-align: left; padding: 10px;">
                        ...
                        <h3 style="color: var(--primary-grad-start); margin: 20px 0 10px;">👑 11. Ad-Free Premium</h3>
                        <ul style="line-height: 1.8; padding-left: 20px;">
                            <li><strong>Why Ads?:</strong> To keep Bunkit free for all students, we display non-intrusive ads</li>
                            <li><strong>Upgrade:</strong> Use the "Buy me a coffee" button to support the developer and unlock Premium</li>
                            <li><strong>Benefits:</strong> No ads anywhere, faster load times, and a cleaner UI</li>
                        </ul>
...
function recommendMedicalCertificates() {
    ...
    if (activeStats.maxPossiblePercent >= minCriteria) {
        ...
        // ML Deficit check
        const maxMLPercentAllowed = Math.max(0, minCriteria - condonationThreshold);
        const maxMLClassesAllowed = Math.round((maxMLPercentAllowed / 100) * activeStats.finalTotal);

        if (deficitClasses > maxMLClassesAllowed) {
            html += `<p style="color: var(--danger-color); font-weight: bold; margin-bottom: 15px;">🛑 Invalid Medical Strategy</p>
                     <p style="color: var(--danger-color); margin-bottom: 15px;">You need <strong>${deficitClasses} class(es)</strong> to reach ${minCriteria}%, but university policy only allows a maximum of ~${maxMLPercentAllowed}% (${maxMLClassesAllowed} classes) covered by Medical Leave for this subject.</p>
                     <p>You cannot use Medical Certificates alone to cover this deficit. You must attend more classes to raise your baseline attendance.</p>
                     </div>`;
            container.innerHTML = html;
            return;
        }
        ...
        html += `<p style="margin-bottom: 15px;">You need to cover a deficit of <strong>${deficitClasses} class(es)</strong> (Max ML Allowed: ${maxMLClassesAllowed}).</p>`;
        ...
        if (bestBlock) {
            ...
            // Build ML Quota dictionary for the active subject
            // If overall, we would need it for all subjects, but recommendMedicalCertificates runs per subject or overall? 
            // Wait, recommendMedicalCertificates currently either calculates overall or per subject.
            // Let's create a dynamic limit checker.
            const mlUsage = {};
            selectedClass.subjects.forEach(s => mlUsage[s.code] = 0);

            const maxMLAllowed = {};
            selectedClass.subjects.forEach(s => {
                const limitStr = getMinMedicalCriteria();
                const stdStr = getMinAttendanceCriteria() * 100;
                const pct = Math.max(0, stdStr - limitStr);
                maxMLAllowed[s.code] = Math.round((pct / 100) * (s.totalHeld + s.remaining));
            });

            // Keep adding dates until we have enough to cover the deficit
            for (const item of validDates) {
                // Break down count from item into valid/invalid
                let validCountPeriod = 0;
                let invalidCountPeriod = 0;
                const d = item.date;
                const dayOfWeek = d.getDay();
                const scheduleIndex = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
                ...
                if (isOverall) {
                    selectedClass.subjects.forEach(sub => {
                        let clsCnt = 0;
                        if (item.isPlannedLeave) {
                            clsCnt = parseScheduleValue(sub.schedule[scheduleIndex]);
                        } else if (logs[item.dateStr]) {
                            const status = logs[item.dateStr][sub.code];
                            if (status === 'Skipped' || status === 'Absent') {
                                clsCnt = parseScheduleValue(sub.schedule[scheduleIndex]);
                            }
                        }
                        ...
                        if (clsCnt > 0) {
                            let available = maxMLAllowed[sub.code] - mlUsage[sub.code];
                            if (available < 0) available = 0;

                            let v = Math.min(clsCnt, available);
                            let inv = clsCnt - v;

                            mlUsage[sub.code] += clsCnt;
                            validCountPeriod += v;
                            invalidCountPeriod += inv;

                            if (inv > 0) {
                                details.push(`${sub.code}: ${v} valid, ${inv} invalid`);
                            }
                        }
                    });
                } else {
                    const subCode = activeStats.code;
                    let available = maxMLAllowed[subCode] - mlUsage[subCode];
                    if (available < 0) available = 0;

                    let v = Math.min(item.count, available);
                    let inv = item.count - v;

                    mlUsage[subCode] += item.count;
                    validCountPeriod += v;
                    invalidCountPeriod += inv;

                    if (inv > 0) {
                        details.push(`${v} valid, ${inv} invalid`);
                    }
                }

                selectedDates.push({
                    ...item,
                    validCount: validCountPeriod,
                    invalidCount: invalidCountPeriod,
                    details: details.join(' | ')
                });

                accumulatedClasses += validCountPeriod; // Only valid counts towards deficit

                // Stop when we have enough valid ML coverage
            ...
            if (accumulatedClasses >= deficitClasses) {
                html += `<div style="background: var(--light-bg); padding: 15px; border-radius: 8px;">
                            ...
                            <p style="font-size: 0.85rem; color: var(--danger-color); margin-bottom: 10px;">⚠️ Some days may have "Invalid" classes if the subject exceeds the maximum ML limit.</p>
                            <ul style="list-style-position: inside; padding-left: 0;">`;
                selectedDates.forEach(d => {
                    let label = d.isPlannedLeave ? `${d.dateStr} 📅 Planned Leave` : `${d.dateStr}`;
                    let countLabel = `${d.validCount} valid ML`;
                    if (d.invalidCount > 0) {
                        countLabel += `, <span style="color:var(--danger-color);">${d.invalidCount} invalid</span> (${d.details})`;
                    }
                    html += `<li>${label} - <strong>${countLabel}</strong></li>`;
                });
                html += `</ul>
...
function calculateMaxSafeLeave() {
    ...
    if (isOverallMode()) {
        ...
        if (bottleneckSubject.maxSkippable < 0) {
            ...
            // --- MKP 0/1 KNAPSACK (BACKTRACKING WITH BRANCH AND BOUND) ---
            // Find absolute maximum number of skippable days using DFS.
            // To prevent UI freeze on large datasets, limit iterations and return best found.

            const subjectKeys = Object.keys(skippableBudget);
            const initialBudgets = subjectKeys.map(k => skippableBudget[k]);

            // Map days array to just their costs for faster array lookups
            const daysData = futureWorkingDays.map(d => ({
                date: d.date,
                costs: subjectKeys.map(k => d.costs[k] || 0)
            }));

            // Optimization: Sort days by total cost ascending to find deep branches faster (greedy initialization)
            daysData.sort((a, b) => {
                const sumA = a.costs.reduce((sum, val) => sum + val, 0);
                const sumB = b.costs.reduce((sum, val) => sum + val, 0);
                return sumA - sumB; // Take cheapest days first to establish a high baseline `maxLen` early on
            });

            let bestSubset = [];
            let maxLen = 0;
            let iterations = 0;
            const MAX_ITERATIONS = 100000; // Hard cap for UI responsiveness

            function findBestSubset(dayIndex, currentSubset, currentBudgets) {
                if (iterations++ > MAX_ITERATIONS) return; // Cutoff

                // If we reached the end, or what's remaining can't possibly beat the current best
                const remainingDays = daysData.length - dayIndex;
                if (currentSubset.length + remainingDays <= maxLen) {
                    return; // Branch bounding
                }

                if (dayIndex >= daysData.length) {
                    if (currentSubset.length > maxLen) {
                        maxLen = currentSubset.length;
                        bestSubset = [...currentSubset];
                    }
                    return;
                }

                const currentDay = daysData[dayIndex];

                // Branch 1: Try taking this day (if budget allows)
                let canTake = true;
                const nextBudgets = new Array(initialBudgets.length);

                for (let i = 0; i < initialBudgets.length; i++) {
                    if (currentDay.costs[i] > currentBudgets[i]) {
                        canTake = false;
                        break;
                    }
                    nextBudgets[i] = currentBudgets[i] - currentDay.costs[i];
                }

                if (canTake) {
                    currentSubset.push(currentDay.date);
                    findBestSubset(dayIndex + 1, currentSubset, nextBudgets);
                    currentSubset.pop(); // backtrack
                }

                // Branch 2: Skip this day
                // Only skip if there's still a mathematical chance to beat `maxLen`
                if (currentSubset.length + remainingDays - 1 > maxLen) {
                    findBestSubset(dayIndex + 1, currentSubset, currentBudgets);
                }
            }

            findBestSubset(0, [], initialBudgets);

            let recommendedLeaveDates = bestSubset;
            recommendedLeaveDates.sort((a, b) => a - b);
...
function calculateMaxSafeLeaveExcluding(excludeCodesStr) {
    ...
    // Map days array to just their costs for faster array lookups
    const daysData = safeDays.map(d => ({
        date: d.date,
        costs: subjectKeys.map(k => d.costs[k] || 0)
    }));

    // Optimization: Sort days by total cost ascending to find deep branches faster
    daysData.sort((a, b) => {
        const sumA = a.costs.reduce((sum, val) => sum + val, 0);
        const sumB = b.costs.reduce((sum, val) => sum + val, 0);
        return sumA - sumB;
    ...
    function findBestSubset(dayIndex, currentSubset, currentBudgets) {
        ...
        const remainingDays = daysData.length - dayIndex;
        if (currentSubset.length + remainingDays <= maxLen) return;
...
function calculateMaxPossibleBunk() {
    ...
    const unreachableSubjects = currentAnalysisData.filter(subject => {
        const finalTotal = subject.totalHeld + subject.remaining;
    ...
    if (unreachableSubjects.length > 0) {
        container.style.display = 'block';
        const subjectNames = unreachableSubjects.map(s => s.name).join(', ');
        container.innerHTML = `
                        <div class="subject-card" style="border-left-color: var(--danger-color);">
    ...
    // Calculate max skippable at Detention threshold
    const skippableBudgetDetention = {};
    const skippableBudgetTarget = {};
    let bottleneckSubjectDetention = { name: '', maxSkippable: Infinity };
    ...
    currentAnalysisData.forEach(subject => {
        ...
        // Calculate at Detention threshold
        const finalTotal = subject.totalHeld + subject.remaining;
        const minRequiredDetention = Math.ceil(detentionCriteria * finalTotal);
        const minRequiredTarget = Math.ceil(minCriteria * finalTotal);

        // Calculate max ML limit to ensure the dropdown to Detention threshold is actually recoverable
        const condonationLimit = getMinMedicalCriteria();
        const standardLimit = getMinAttendanceCriteria() * 100;
        const maxMLPercentAllowed = Math.max(0, standardLimit - condonationLimit);

        // Subtract Cancelled classes logged by the user from finalTotal
        let cancelledClasses = 0;
        const logs = JSON.parse(localStorage.getItem('attendance_logs')) || {};
        Object.keys(logs).forEach(dateStr => {
            const dayLog = logs[dateStr];
            Object.keys(dayLog).forEach(key => {
                const logSubCode = key.split('_p')[0];
                if (logSubCode === subject.code && dayLog[key] === 'Cancelled') {
                    const isLegacyFormat = !key.includes('_p');
                    cancelledClasses += isLegacyFormat ? getSubjectClassCountForDate(dateStr, subject.code) : 1;
                }
            });
        });

        const trueFinalTotal = Math.max(0, finalTotal - cancelledClasses);
        const maxMLClassesAllowed = Math.round((maxMLPercentAllowed / 100) * trueFinalTotal);

        const absoluteMinRequired = minRequiredTarget - maxMLClassesAllowed;
        const actualMinRequiredToRecover = Math.max(minRequiredDetention, absoluteMinRequired);

        const stillNeedDetention = Math.max(0, actualMinRequiredToRecover - subject.attended);
        const maxSkippableDetention = Math.max(0, subject.remaining - stillNeedDetention) - leaveCost;
        skippableBudgetDetention[subject.code] = maxSkippableDetention;

        // Calculate at Target threshold (for ML suggestions)
        const stillNeedTarget = Math.max(0, minRequiredTarget - subject.attended);
        const maxSkippableTarget = Math.max(0, subject.remaining - stillNeedTarget) - leaveCost;
        skippableBudgetTarget[subject.code] = maxSkippableTarget;

        if (maxSkippableDetention < bottleneckSubjectDetention.maxSkippable) {
            bottleneckSubjectDetention = { name: subject.name, maxSkippable: maxSkippableDetention };
        }
    });

    // --- MKP 0/1 KNAPSACK (BACKTRACKING WITH BRANCH AND BOUND) ---
    const subjectKeysDetention = Object.keys(skippableBudgetDetention);
    const initialBudgetsDetention = subjectKeysDetention.map(k => skippableBudgetDetention[k]);

    const daysDataDetention = futureWorkingDays.map(d => ({
        date: d.date,
        costsObj: d.costs,
        costs: subjectKeysDetention.map(k => d.costs[k] || 0)
    }));

    daysDataDetention.sort((a, b) => {
        const sumA = a.costs.reduce((sum, val) => sum + val, 0);
        const sumB = b.costs.reduce((sum, val) => sum + val, 0);
        return sumA - sumB;
    });

    let bestSubsetDetention = [];
    let maxLenDetention = 0;
    let iterationsDetention = 0;
    const MAX_ITERATIONS_DETENTION = 100000;

    function findBestSubsetDetention(dayIndex, currentSubset, currentBudgets) {
        if (iterationsDetention++ > MAX_ITERATIONS_DETENTION) return;

        const remainingDays = daysDataDetention.length - dayIndex;
        if (currentSubset.length + remainingDays <= maxLenDetention) return;

        if (dayIndex >= daysDataDetention.length) {
            if (currentSubset.length > maxLenDetention) {
                maxLenDetention = currentSubset.length;
                bestSubsetDetention = [...currentSubset];
            }
            return;
        }

        const currentDay = daysDataDetention[dayIndex];

        // Take
        let canTake = true;
        const nextBudgets = new Array(initialBudgetsDetention.length);
        for (let i = 0; i < initialBudgetsDetention.length; i++) {
            if (currentDay.costs[i] > currentBudgets[i]) {
                canTake = false;
                break;
            }
        ...
        if (canTake) {
            currentSubset.push({ date: currentDay.date, costs: currentDay.costsObj });
            findBestSubsetDetention(dayIndex + 1, currentSubset, nextBudgets);
            currentSubset.pop();
        }

        // Skip
        if (currentSubset.length + remainingDays - 1 > maxLenDetention) {
            findBestSubsetDetention(dayIndex + 1, currentSubset, currentBudgets);
        }
    }

    findBestSubsetDetention(0, [], initialBudgetsDetention);

    let bunkableDates = bestSubsetDetention;
    bunkableDates.sort((a, b) => a.date - b.date);

    // Calculate classes needed via ML to go from Detention threshold to Target threshold
    let mlDatesNeeded = [];
    ...
    currentAnalysisData.forEach(subject => {
        ...
        // If projected < Target %, calculate ML classes needed
        if (projectedPercent < minCriteria * 100) {
            const deficit = Math.ceil(minCriteria * finalTotal) - projectedAttended;

            // Check against university policy limit (e.g. 10%)
            const condonationLimit = getMinMedicalCriteria();
            const standardLimit = getMinAttendanceCriteria() * 100;
            const maxMLPercentAllowed = Math.max(0, standardLimit - condonationLimit);

            // Subtract Cancelled classes again
            let cancelledClassesDeficitCheck = 0;
            const logsCheck = JSON.parse(localStorage.getItem('attendance_logs')) || {};
            Object.keys(logsCheck).forEach(dateStr => {
                const dayLog = logsCheck[dateStr];
                Object.keys(dayLog).forEach(key => {
                    const logSubCode = key.split('_p')[0];
                    if (logSubCode === subject.code && dayLog[key] === 'Cancelled') {
                        const isLegacyFormat = !key.includes('_p');
                        cancelledClassesDeficitCheck += isLegacyFormat ? getSubjectClassCountForDate(dateStr, subject.code) : 1;
                    }
                });
            });
            const trueFinalTotalDeficit = Math.max(0, finalTotal - cancelledClassesDeficitCheck);
            const maxMLClassesAllowed = Math.round((maxMLPercentAllowed / 100) * trueFinalTotalDeficit);

            if (deficit > 0) {
                mlDatesNeeded.push({ subject: subject.name, code: subject.code, classesNeeded: deficit, exceedsLimit: deficit > maxMLClassesAllowed });
    ...
    // Sort by class volume
    availableMLDates.sort((a, b) => b.classes - a.classes);

    // Select enough ML dates to cover the deficit while tracking ML Subject Quotas
    let selectedMLDates = [];
    let coveredClasses = 0;

    // Build ML Quota dictionary
    const mlUsage = {};
    const maxMLAllowed = {};
    currentAnalysisData.forEach(s => {
        mlUsage[s.code] = 0;
        const limitStr = getMinMedicalCriteria();
        const stdStr = getMinAttendanceCriteria() * 100;
        const pct = Math.max(0, stdStr - limitStr);

        let cancelledClassesQuota = 0;
        const logsQuota = JSON.parse(localStorage.getItem('attendance_logs')) || {};
        Object.keys(logsQuota).forEach(dateStr => {
            const dayLog = logsQuota[dateStr];
            Object.keys(dayLog).forEach(key => {
                const logSubCode = key.split('_p')[0];
                if (logSubCode === s.code && dayLog[key] === 'Cancelled') {
                    const isLegacyFormat = !key.includes('_p');
                    cancelledClassesQuota += isLegacyFormat ? getSubjectClassCountForDate(dateStr, s.code) : 1;
                }
            });
        });

        const finalTrueClasses = Math.max(0, (s.totalHeld + s.remaining) - cancelledClassesQuota);
        maxMLAllowed[s.code] = Math.round((pct / 100) * finalTrueClasses);
    });

    for (const mlDate of availableMLDates) {
        if (coveredClasses >= totalMLClassesNeeded) break;

        // Find which subjects have classes on this date
        let subjectsToday = [];
        if (mlDate.source === 'bunk') {
            // Re-find from bunk costs
            const theBunk = bunkableDates.find(b => b.date.getTime() === mlDate.date.getTime());
            if (theBunk) {
                Object.keys(theBunk.costs).forEach(code => {
                    if (theBunk.costs[code] > 0) subjectsToday.push({ code, count: theBunk.costs[code] });
                });
            }
        } else if (mlDate.source === 'planned') {
            const dayOfWeek = mlDate.date.getDay();
            const scheduleIndex = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
            currentAnalysisData.forEach(subject => {
                let cnt = parseScheduleValue(subject.schedule[scheduleIndex]);
                if (cnt > 0) subjectsToday.push({ code: subject.code, count: cnt });
            });
        }
        ...
        subjectsToday.forEach(sub => {
            ...
            let v = Math.min(sub.count, available);
            let inv = sub.count - v;

            mlUsage[sub.code] += sub.count;
            validCount += v;
            invalidCount += inv;

            if (inv > 0) details.push(`${sub.code}: ${v} valid, ${inv} invalid`);
        });

        selectedMLDates.push({
            ...mlDate,
            validCount,
            invalidCount,
            details: details.join(' | ')
    ...
    let resultHTML = `
                    <div class="subject-card" style="border-left-color: var(--danger-color);">
                        ...
                        <p style="margin-bottom: 15px; padding: 10px; background: rgba(231, 76, 60, 0.1); border-radius: 8px;">
                            <strong>⚠️ Important:</strong> These are dates you can skip without falling below <strong>${detentionPercent}%</strong> (detention threshold). Bottleneck: <strong>${bottleneckSubjectDetention.name}</strong>
                        </p>`;
        ...
        resultHTML += `
                        <div style="margin-top: 20px; padding: 15px; background: rgba(52, 152, 219, 0.1); border-radius: 8px; border-left: 4px solid var(--info-color);">
                            ...
                            <ul style="list-style-position: inside; padding: 10px 0;">
                                ${mlDatesNeeded.map(m => {
            if (m.exceedsLimit) {
                return `<li style="color: var(--danger-color);"><strong>${m.subject}</strong>: ${m.classesNeeded} class(es) needed <em>(⚠️ Policy allows max ~10%)</em></li>`;
            }
            return `<li><strong>${m.subject}</strong>: ${m.classesNeeded} class(es) needed</li>`;
        }).join('')}
                            </ul>
                            <p style="margin-top: 10px;"><strong>Total valid classes to cover via ML:</strong> ${totalMLClassesNeeded}</p>`;

        if (selectedMLDates.length > 0 && coveredClasses >= totalMLClassesNeeded) {
            resultHTML += `
                            <h4 style="margin-top: 15px;">📋 Recommended ML Dates (${selectedMLDates.length} days)</h4>
                            <p style="font-size: 0.85rem; color: var(--danger-color); margin-bottom: 10px;">⚠️ Some days may have "Invalid" classes if the subject exceeds the maximum ML limit.</p>
                            <ul style="list-style-position: inside; padding: 10px 0;">
                                ${selectedMLDates.map(d => {
                let countLabel = `${d.validCount} valid ML`;
                if (d.invalidCount > 0) {
                    countLabel += `, <span style="color:var(--danger-color);">${d.invalidCount} invalid</span> (${d.details})`;
                }
                return `<li>${d.date.toDateString()} - <strong>${countLabel}</strong> <em>(${d.source === 'bunk' ? 'From Bunk List' : 'From Planned Leave'})</em></li>`;
            }).join('')}
                            </ul>`;
        } else if (selectedMLDates.length > 0) {
            resultHTML += `
                            <p style="color: var(--danger-color); margin-top: 10px; font-weight: bold;">
                                ⚠️ Insufficient Medical Quota!
                            </p>
                            <p style="color: var(--warning-color); font-size: 0.9rem;">
                                Available dates can only supply ${coveredClasses} valid ML classes out of the required ${totalMLClassesNeeded}. Further absences will lead to detention.
                            </p>`;
        } else {
...
function saveDailyLog() {
    ...
    // Validation: Don't allow saving if NO subjects were found/displayed
    const subjectSelects = document.querySelectorAll('#dailyLogSubjects select');
    if (subjectSelects.length === 0) {
        alert('❌ Error: No subjects found to save for this day. Please check your schedule/custom schedule.');
        return;
    }
    ...
    if (Object.keys(newDayLog).length > 0) {
        ...
        // BUT ONLY if we actually have subject selects present (to distinguish between "marking none" vs "UI broken")
        delete logs[dateStr];
    }
...
function getBackupDataObj() {
    ...
    return {
        ...
        calcSettings: {
            minAttendance: localStorage.getItem('calcSettings_minAttendance') || '75',
            minMedical: localStorage.getItem('calcSettings_minMedical') || '65',
            isOverall: localStorage.getItem('calcSettings_isOverall') || 'false'
        },
...
function processRestoreData(jsonString) {
    ...
    if (isCompleteBackup) {
        ...
        if (backupData.calcSettings) {
            if (backupData.calcSettings.minAttendance) {
                localStorage.setItem('calcSettings_minAttendance', backupData.calcSettings.minAttendance);
            }
            if (backupData.calcSettings.minMedical) {
                localStorage.setItem('calcSettings_minMedical', backupData.calcSettings.minMedical);
            }
            if (backupData.calcSettings.isOverall) {
                localStorage.setItem('calcSettings_isOverall', backupData.calcSettings.isOverall);
            }
        }
...
async function deleteSelectedClass() {
    ...
    if (confirm(`Are you sure you want to permanently delete the class "${className}"? This cannot be undone.`)) {
        ...
        if (selector.options[selector.selectedIndex]) {
            selector.options[selector.selectedIndex].text = 'Deleting...';
        }
        document.body.style.cursor = 'wait';

        try {
            // 1. Delete from Cloud FIRST (if logged in and online)
            if (window.SyncManager && window.SyncManager.deleteClass) {
                console.log(`[Sync] Deleting class "${className}" from cloud...`);
                // Use a short timeout to prevent hanging forever if offline but authenticated
                const deletePromise = SyncManager.deleteClass(className);
                const timeoutPromise = new Promise(resolve => setTimeout(() => resolve('timeout'), 5000));

                const result = await Promise.race([deletePromise, timeoutPromise]);
                if (result === 'timeout') {
                    console.warn(`[Sync] Cloud deletion timed out. Proceeding with local delete. Tombstone saved.`);
                } else {
                    console.log(`[Sync] Cloud deletion resolved.`);
                }
            }
        } catch (e) {
            console.error(`[Sync] Failed to delete from cloud, but proceeding locally:`, e);
        }
        ...
        localStorage.removeItem(`timetable_history_${className}`);

        // Since BunkIt is designed for 1 class per semester, we should also wipe global logs to ensure a fresh start
        localStorage.removeItem('attendance_logs');
        localStorage.removeItem('selectedClass');
        localStorage.removeItem('classesData');
        localStorage.removeItem('classesListCache');
        localStorage.removeItem('lastOpenedClass');
        localStorage.removeItem('ocrResultsCache');
        // Force sync timestamp to clear local data precedence
        localStorage.removeItem('attendance_log_timestamps');

        // SCORCHED EARTH: Also wipe from Service Worker IndexedDB so it cannot resurrect
        try {
            const dbRequest = indexedDB.open('BunkitDB', 1);
            dbRequest.onsuccess = (event) => {
                const db = event.target.result;
                if (db.objectStoreNames.contains('settings')) {
                    const tx = db.transaction(['settings'], 'readwrite');
                    const store = tx.objectStore('settings');
                    store.delete('attendance_logs');
                    store.delete('classesData');
                    store.delete('selectedClass');
                }
                // Force a clean reload AFTER IndexedDB is wiped
...
function calculateFromPortal(previewOverrides = null) {
    ...
    // 5. Calculate theoretical max classes per subject from semester start to effective current date
    ...
    selectedClass.subjects.forEach(s => { maxPossibleClasses[s.code.toUpperCase()] = 0; });

    // Exhaustive mapping for any log keys that aren't in subjects array
    Object.values(logs).forEach(dayLog => {
        Object.keys(dayLog).forEach(key => {
            const normKey = key.split('_p')[0].toUpperCase();
            if (maxPossibleClasses[normKey] === undefined) {
                maxPossibleClasses[normKey] = 0;
            }
        });
    });

    let countDate = new Date(semesterStart);
    while (countDate <= effectiveEnd) {
        const dateStr = formatLocalDate(countDate);
    ...
    // Calculate MAX ML PERMITTED (10%)
    const maxMLAllowed = {};
    const condonationLimit = getMinMedicalCriteria();
    const standardLimit = getMinAttendanceCriteria() * 100;
    const maxMLPercentAllowed = Math.max(0, standardLimit - condonationLimit);
    const useOverallMode = isOverallMode();

    // Subtract Cancelled classes to narrow the ML cap dynamically for the whole portal
    // Subtract Cancelled classes from maxPossibleClasses to dynamically narrow ML cap
    Object.keys(logs).forEach(dateStr => {
        const dayLog = logs[dateStr];
        Object.keys(dayLog).forEach(key => {
            if (dayLog[key] === 'Cancelled') {
                const rawCode = key.split('_p')[0];
                const normCode = rawCode.toUpperCase();
                const isLegacyFormat = !key.includes('_p');
                const classCount = isLegacyFormat ? getSubjectClassCountForDate(dateStr, rawCode) : 1;
                if (maxPossibleClasses[normCode] !== undefined) {
                    maxPossibleClasses[normCode] = Math.max(0, maxPossibleClasses[normCode] - classCount);
                }
            }
        });
    });
    ...
    if (useOverallMode) {
        // OVERALL MODE: Pool all subjects into one combined limit
        const totalMaxClasses = Object.values(maxPossibleClasses).reduce((sum, v) => sum + v, 0);
        const overallLimit = Math.floor((maxMLPercentAllowed / 100) * totalMaxClasses);
        selectedClass.subjects.forEach(s => {
            maxMLAllowed[s.code.toUpperCase()] = overallLimit;
        });
        Object.keys(maxPossibleClasses).forEach(normKey => {
            if (maxMLAllowed[normKey] === undefined) maxMLAllowed[normKey] = overallLimit;
        });
        maxMLAllowed['_OVERALL'] = overallLimit;
    } else {
        selectedClass.subjects.forEach(s => {
            const normCode = s.code.toUpperCase();
            maxMLAllowed[normCode] = Math.floor((maxMLPercentAllowed / 100) * (maxPossibleClasses[normCode] || 0));
        });
        Object.keys(maxPossibleClasses).forEach(normKey => {
            if (maxMLAllowed[normKey] === undefined) {
                maxMLAllowed[normKey] = Math.floor((maxMLPercentAllowed / 100) * (maxPossibleClasses[normKey] || 0));
            }
        });
    }
    ...
    // Track ML per subject for chronological capping
    const mlCounts = {};
    if (useOverallMode) {
        mlCounts['_OVERALL'] = 0;
    } else {
        selectedClass.subjects.forEach(s => { mlCounts[s.code.toUpperCase()] = 0; });
        Object.keys(maxPossibleClasses).forEach(normKey => {
            if (mlCounts[normKey] === undefined) mlCounts[normKey] = 0;
        });
    }

    // SORT dates chronologically to apply ML limits properly
    ...
    sortedLogDates.forEach(dateStr => {
        ...
        if (logDate > baselineDate && logDate <= effectiveDateObj) {
            ...
            Object.keys(dayLog).forEach(key => {
                ...
                if (effectiveStats[rawCode]) {
                    const status = dayLog[key];
                    const s = String(status || '').toUpperCase().trim();
                    let isPresent = status === 'Attended' || status === 'Present' || status === 'Duty Leave (OD)' || s === 'P' || s === 'PRESENT' || s === 'ATTENDED' || s === 'OD' || s === 'DUTY LEAVE (OD)';
                    const isMedical = status === 'Medical Leave (ML)' || s === 'ML' || s === 'MEDICAL LEAVE (ML)';
                    let isAbsent = status === 'Skipped' || status === 'Absent' || s === 'A' || s === 'ABSENT' || s === 'SKIPPED';

                    const classCount = isLegacyFormat ? getSubjectClassCountForDate(dateStr, rawCode) : 1;
                    const countKey = useOverallMode ? '_OVERALL' : normCode;

                    if (isMedical) {
                        const limit = maxMLAllowed[countKey];
                        if (mlCounts[countKey] !== undefined && mlCounts[countKey] + classCount <= limit) {
                            isPresent = true;
                            mlCounts[countKey] += classCount;
                        } else {
                            // INVALID ML -> Counts as absent
                            isAbsent = true;
                        }
                    }

                    if (isPresent) {
                        effectiveStats[rawCode].attended += classCount;
                        effectiveStats[rawCode].total += classCount;
                    } else if (isAbsent) {
                        effectiveStats[rawCode].total += classCount;
                    }
                }
    ...
    // 4. Cap effectiveStats at max possible classes
    selectedClass.subjects.forEach(subject => {
        const code = subject.code;
        const normCode = code.toUpperCase();
        const maxClasses = maxPossibleClasses[normCode] || 0;
...
function applyBulkAction(status) {
    ...
    allSelects.forEach(select => {
        if (status === 'Medical Leave (ML)' && select.value === 'Cancelled') {
            return; // Skip cancelled classes when applying bulk ML
        }
        select.value = status;
    ...
    if (status === 'Medical Leave (ML)') {
        alert("All applicable subjects marked as Medical Leave (ML).");
    } else if (status === 'Cancelled') {
        alert("All subjects marked as Cancelled.");
    } else if (status === 'Default') {
        alert("All subjects reset to Default.");
    }
}
...
function handleStatusChange(selectElement) {
    ...
    if (status === 'Medical Leave (ML)') {
        // Auto-set all others to ML except Cancelled
        allSelects.forEach(select => {
        ...
        // Ensure the actively clicked item is definitely ML (overriding even Cancelled if explicitly clicked)
        selectElement.value = 'Medical Leave (ML)';
        selectElement.dataset.previousValue = 'Medical Leave (ML)';
    } else if (wasML && status !== 'Medical Leave (ML)') {
        // If switching from ML to Cancelled, leave other MLs alone (mixing allowed)
        if (status === 'Cancelled') {
            selectElement.dataset.previousValue = status;
        } else {
            // Changing FROM ML to something else (Attended/Skipped) - reset all other MLs to Default
            allSelects.forEach(select => {
                if (select.value === 'Medical Leave (ML)') {
                    select.value = 'Default';
                    select.dataset.previousValue = 'Default';
                }
            });
            // Re-apply the targeted status
...
function onScanSuccessTab(decodedText) {
    try {
        // --- NEW: Check if the scanned text is a Bunkit URL with shared_class_id or class ---
        if (decodedText.startsWith('http://') || decodedText.startsWith('https://')) {
            const url = new URL(decodedText);
            const sharedClassId = url.searchParams.get('shared_class_id');
            const classParam = url.searchParams.get('class');

            if (sharedClassId) {
                // FIX 6: Stop scanner before importing
                stopTabScanner();
                importSharedClassFromDB(sharedClassId);
                return;
            } else if (classParam) {
                // FIX 6: Stop scanner before importing
                stopTabScanner();
                importClassFromURL(classParam);
                return;
            }
        }

        // --- OLD: Fallback to direct JSON parsing ---
...
async function getOrCreateSharedClassLink(className) {
    if (!className || !classes[className]) return null;
    const currentClass = classes[className];

    // Ensure we have a sharedId
    if (!currentClass.sharedId) {
        if (window.SocialManager) {
            currentClass.sharedId = await window.SocialManager.generateSharedClassId(currentClass);
            saveToStorage();
        } else {
            // Fallback unique ID
            const cryptoPart = window.crypto && window.crypto.randomUUID ? window.crypto.randomUUID().split('-')[0] : Math.random().toString(36).substring(2, 10);
            currentClass.sharedId = 'local_' + cryptoPart;
            saveToStorage();
        }
    }
    ...
    const cleanClassData = {
        ...
        timetable: getTimetableArrangement(className) || {},
        periodTimes: currentClass.periodTimes || {},
        lastDate: currentClass.lastDate || '',
        startDate: (currentClass.portalSetup && currentClass.portalSetup.semesterStartDate) || ''
    };

    // FIX 4 & 5: Upsert into Supabase shared_classes table (fail loudly)
    if (window.supabaseClient) {
        try {
            const { error } = await window.supabaseClient.from('shared_classes').upsert({
                id: currentClass.sharedId,
                class_data: cleanClassData
            });
            if (error) {
                console.error("Failed to store class in DB:", error);
                const msg = (error.message || '').toLowerCase();
                if (error.code === '42P01' || msg.includes('relation') || msg.includes('does not exist')) {
                    showToast('❌ Database table "shared_classes" not found. Please run the SQL setup script in Supabase.', 'error');
                } else {
                    showToast(`❌ Failed to save class to cloud: ${error.message}`, 'error');
                }
                return null; // Signal failure — don't return a URL that won't work
            }
        } catch (e) {
            console.error("Supabase upsert failed:", e);
            showToast('❌ Failed to connect to cloud. Share link may not work for others.', 'error');
            return null; // Signal failure
        }
    } else {
        showToast('⚠️ Cloud sync not available. Share link may not work for others.', 'warning');
        return null; // No supabase, can't share via DB
    }

    // Generate Share URL
    const shareUrl = `${window.location.origin}${window.location.pathname}?shared_class_id=${currentClass.sharedId}`;
    return shareUrl;
}
...
async function generateShareQRTab() {
    ...
    qrContainer.innerHTML = '<p style="text-align:center;padding:20px;">⏳ Generating QR Code...</p>';

    try {
        const shareUrl = await getOrCreateSharedClassLink(editingClassName);
...
async function shareClassLinkFromTab() {
    ...
    let shareUrl;
    try {
        shareUrl = await getOrCreateSharedClassLink(editingClassName);
    } catch (e) {
        console.error(e);
    ...
    const shareData = {
        title: `${editingClassName} | Bunkit Class`,
        text: `${editingClassName} | Bunkit Class\n\n` +
...
function getSubjectColorsForPeriodView() {
    ...
    selectedClass.subjects.forEach((s, i) => {
        ...
        const normCode = String(s.code || '').toUpperCase();
        colors[normCode] = {
            glow: baseColor,
            name: s.name,
            index: i
...
function getTimetableForDay(dayIndex) {
    ...
    if (arrangement && arrangement[dayIndex]) {
        ...
        return arrangement[dayIndex].map(item => {
            ...
            const normCode = String(rawCode).toUpperCase();

            // Return a standardized object
            if (typeof item === 'object') {
                return { ...item, code: normCode };
            }
        ...
        selectedClass.subjects.forEach(subject => {
            ...
            for (let i = 0; i < count; i++) {
                periods.push({ code: String(subject.code).toUpperCase(), name: subject.name });
            }
        });
        return periods;
    }
...
// --- GLOBAL STATE for period view preservation ---
let _periodViewState = null;

function savePeriodViewState() {
    const periodModal = document.getElementById('periodAttendanceModal');
    if (!periodModal) return;

    const monthSections = document.querySelectorAll('.period-month-section');
    const collapsedMonths = [];
    monthSections.forEach((sec) => {
        const monthId = sec.getAttribute('data-month-id');
        if (sec.classList.contains('collapsed') && monthId) {
            collapsedMonths.push(monthId);
        }
    });

    // Save scroll from both modal container and content wrapper to cover mobile browser quirks
    const contentContainer = periodModal.querySelector('.modal-content');
    _periodViewState = {
        modalScrollTop: periodModal.scrollTop || 0,
        contentScrollTop: contentContainer ? (contentContainer.scrollTop || 0) : 0,
        collapsedMonths: collapsedMonths,
        timestamp: Date.now(),
        hasExplicitState: true // flag to differentiate saved state vs initial open
    };
}
...
function openPeriodAttendanceModal() {
    ...
    // --- PRESERVE SCROLL & COLLAPSE STATE ---
    let savedModalScroll = 0;
    let savedContentScroll = 0;
    let savedCollapsedMonths = [];
    const periodModal = document.getElementById('periodAttendanceModal');
    const isModalAlreadyOpen = periodModal && periodModal.classList.contains('active');
    let stateWasRestored = false;

    if (_periodViewState && _periodViewState.hasExplicitState) {
        // Use explicit state captured just before action
        savedModalScroll = _periodViewState.modalScrollTop || 0;
        savedContentScroll = _periodViewState.contentScrollTop || 0;
        savedCollapsedMonths = _periodViewState.collapsedMonths || [];
        stateWasRestored = true;
        _periodViewState = null; // consume it
    } else if (isModalAlreadyOpen) {
        // Modal is actively being viewed but explicit state was lost. Force DOM read.
        savedModalScroll = periodModal.scrollTop || 0;
        const existingModalContent = periodModal.querySelector('.modal-content');
        if (existingModalContent) savedContentScroll = existingModalContent.scrollTop || 0;

        const monthSections = periodModal.querySelectorAll('.period-month-section');
        monthSections.forEach((sec) => {
            if (sec.classList.contains('collapsed')) {
                const monthId = sec.getAttribute('data-month-id');
                if (monthId) savedCollapsedMonths.push(monthId);
            }
        });
        stateWasRestored = true;
    }
    ...
    const startDate = parseLocalDate(selectedClass.portalSetup.semesterStartDate);
    const baselineDate = selectedClass?.portalSetup?.baselineDate ? parseLocalDate(selectedClass.portalSetup.baselineDate) : null;
    ...
    // Get max classes calculation
    const maxPossibleClasses = {};
    const effectiveEnd = new Date();
    const holidays = selectedClass.holidays || [];
    selectedClass.subjects.forEach(s => {
        const normCode = s.code.toUpperCase();
        maxPossibleClasses[normCode] = 0;
    });
    ...
    while (countDate <= effectiveEnd) {
        ...
        if (!holidays.includes(dateStr)) {
            selectedClass.subjects.forEach(subject => {
                const normCode = subject.code.toUpperCase();
                const count = getSubjectClassCountForDate(dateStr, subject.code);
                if (maxPossibleClasses[normCode] !== undefined) {
                    maxPossibleClasses[normCode] += count;
                }
            });
        }
        countDate.setDate(countDate.getDate() + 1);
    ...
    // Subtract Cancelled classes to narrow the ML cap dynamically
    Object.keys(logs).forEach(dateStr => {
        const dayLog = logs[dateStr];
        Object.keys(dayLog).forEach(key => {
            if (dayLog[key] === 'Cancelled') {
    ...
    // Calculate ML limits based on mode
    if (useOverallMode) {
        const totalMaxClasses = Object.values(maxPossibleClasses).reduce((sum, v) => sum + v, 0);
        const overallLimit = Math.round((maxMLPercentAllowed / 100) * totalMaxClasses);
        selectedClass.subjects.forEach(s => {
            maxMLAllowed[s.code.toUpperCase()] = overallLimit;
        });
        Object.keys(maxPossibleClasses).forEach(normKey => {
        ...
        selectedClass.subjects.forEach(s => {
            const normCode = s.code.toUpperCase();
            maxMLAllowed[normCode] = Math.round((maxMLPercentAllowed / 100) * maxPossibleClasses[normCode]);
        });
        Object.keys(maxPossibleClasses).forEach(normKey => {
            if (maxMLAllowed[normKey] === undefined) maxMLAllowed[normKey] = Math.round((maxMLPercentAllowed / 100) * maxPossibleClasses[normKey]);
        });
    }

    // Track ML counts sequentially
    const mlCounts = {};
    if (useOverallMode) {
        mlCounts['_OVERALL'] = 0;
    } else {
    ...
    // Pre-calculate valid/invalid ML
    const mlValidityCache = {};
    Object.keys(logs).sort().forEach(dateStr => {
        mlValidityCache[dateStr] = {};
        const logDate = parseLocalDate(dateStr);
        if (baselineDate && logDate <= baselineDate) return;

        const dayLog = logs[dateStr];
        Object.keys(dayLog).forEach(key => {
            const status = dayLog[key];
            const sStat = String(status || '').toUpperCase().trim();
            const isMedical = status === 'Medical Leave (ML)' || sStat === 'ML' || sStat === 'MEDICAL LEAVE (ML)';
            if (!isMedical) return;
            ...
            if (mlCounts[countKey] !== undefined) {
                const classCount = isLegacyFormat ? getSubjectClassCountForDate(dateStr, rawCode) : 1;
                const limit = maxMLAllowed[countKey];

                if (mlCounts[countKey] + classCount <= limit) {
                    mlValidityCache[dateStr][key.toUpperCase()] = true;
                    mlCounts[countKey] += classCount;
                } else {
                    mlValidityCache[dateStr][key.toUpperCase()] = false;
                }
            }
        });
    });
    // ---------------------------------
    console.log('[ML Debug] mode:', useOverallMode ? 'OVERALL' : 'PER-SUBJECT', 'maxMLAllowed:', JSON.stringify(maxMLAllowed), 'mlCounts:', JSON.stringify(mlCounts), 'maxMLPercentAllowed:', maxMLPercentAllowed);
    Object.keys(mlValidityCache).forEach(d => {
        const entries = mlValidityCache[d];
        if (Object.keys(entries).length > 0) console.log('[ML Debug]', d, JSON.stringify(entries));
    });
    ...
    Object.keys(monthGroups).sort().reverse().forEach((monthKey, idx) => {
        ...
        if (stateWasRestored) {
            // Intelligently restore exact state: collapse if it was explicitly collapsed
            isCollapsed = savedCollapsedMonths.includes(monthKey);
        } else if (isModalAlreadyOpen) {
            // Failsafe: if state failed but modal is open, NEVER force collapse on the user
            isCollapsed = false;
        } else {
            // Default behavior (first open): only expand the most recent month
            isCollapsed = (idx > 0);
        }
        const collapsedClass = isCollapsed ? 'collapsed' : '';

        modalHTML += `
                        <div class="period-month-section ${collapsedClass}" data-month-id="${monthKey}">
        ...
        month.dates.forEach(date => {
            ...
            // Add bulk action triggers to the date cell (Right-click or Long-press)
            modalHTML += `<div class="period-grid-row">
                            <div class="period-date-cell" 
                                 title="Hold, Click or Right-Click for Bulk Actions"
                                 onclick="openPeriodBulkActionPopup(event, '${dateStr}')"
                                 oncontextmenu="openPeriodBulkActionPopup(event, '${dateStr}')"
                                 onmousedown="startPeriodLongPress(event, '${dateStr}')" 
                                 onmouseup="cancelPeriodLongPress()"
                                 onmouseleave="cancelPeriodLongPress()"
                                 ontouchstart="startPeriodLongPress(event, '${dateStr}')"
                                 ontouchend="cancelPeriodLongPress()"
                                 ontouchcancel="cancelPeriodLongPress()">
            ...
            for (let p = 0; p < maxPeriods; p++) {
                if (isHoliday) {
                    ...
                    const specificBaseCode = codeToUse ? codeToUse.split('_p')[0] : null;
                    ...
                    else if (status === 'Medical Leave (ML)' || String(status).toUpperCase() === 'ML') {
                        // Check validity cache - try specific key first, then fall back to base code
                        // updatePeriodStatus stores ML under base code (e.g., MATH) but we render with editKey (e.g., MATH_p1)
                        let mlIsInvalid = false;
                        if (mlValidityCache[dateStr]) {
                            const normKey = codeToUse.toUpperCase();
                            const normBase = specificBaseCode ? specificBaseCode.toUpperCase() : null;
                            if (mlValidityCache[dateStr][codeToUse] === false || mlValidityCache[dateStr][normKey] === false) {
                                mlIsInvalid = true;
                            } else if (mlValidityCache[dateStr][codeToUse] === undefined && normBase && mlValidityCache[dateStr][normBase] === false) {
                                mlIsInvalid = true;
                            }
                        }
                        if (mlIsInvalid) {
                            cellClass = 'ml ml-invalid';
                            symbol = 'ML⚠️';
                            warningTitle = '❌ Unusable ML (limit exceeded). Counts as Absent.';
                        } else {
                            cellClass = 'ml';
                            symbol = 'ML';
                            warningTitle = '✓ Usable ML';
                        }
                    }

                    // Glow effect using subject color
                    const glowColor = subjectColor ? subjectColor.glow : '#667eea';
                    let glowStyle = `box-shadow: 0 0 8px 2px ${glowColor}40, inset 0 0 0 2px ${glowColor};`;

                    if (cellClass.includes('ml-invalid')) {
                        glowStyle = `box-shadow: 0 0 10px 3px rgba(255, 59, 48, 0.4), inset 0 0 0 2px rgba(255, 59, 48, 1); color: white !important; background: #ff3b30 !important; font-weight: bold;`;
                    } else if (cellClass === 'ml') {
                        // Distinct Yellow coloring for Usable Medical Leaves
                        glowStyle = `box-shadow: 0 0 8px 2px #FF980060, inset 0 0 0 2px #FF9800; color: #FF9800 !important; background: rgba(255, 152, 0, 0.1) !important;`;
                    }
                    ...
                    modalHTML += `<div class="period-cell ${cellClass}" 
                                    ...
                                    data-code="${codeToUse}"
                                    data-period="${p}"
                                    title="${period.name} | ${status} ${warningTitle}"
    ...
    // --- RESTORE SCROLL (COLLAPSE STATE ALREADY APPLIED IN HTML) ---
    if (stateWasRestored || savedModalScroll > 0 || savedContentScroll > 0) {
        // STEP 2: Restore scroll
        const targetModal = document.getElementById('periodAttendanceModal');
        const targetModalContent = targetModal ? targetModal.querySelector('.modal-content') : null;

        if (targetModal || targetModalContent) {
            // Force a reflow so the browser recalculates heights after collapse changes
            if (targetModalContent) void targetModalContent.offsetHeight;

            const restoreScrollAction = () => {
                if (targetModal && savedModalScroll > 0) targetModal.scrollTop = savedModalScroll;
                if (targetModalContent && savedContentScroll > 0) targetModalContent.scrollTop = savedContentScroll;
            };

            // Restore in multiple frames to ensure it sticks as content renders
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    restoreScrollAction();
                    setTimeout(restoreScrollAction, 30);
                    setTimeout(restoreScrollAction, 100);
                    setTimeout(restoreScrollAction, 250);
                });
            });
        }
    }
...
function openPeriodEditPopup(event, dateStr, code, subjectName) {
    ...
    // Position near click but prevent off-screen overflow
    const rect = event.target.getBoundingClientRect();
    const popupHeight = 260; // Approximate height of the menu
    const margin = 10;

    let topPos = rect.bottom + 5;
    if (topPos + popupHeight > window.innerHeight) {
        topPos = Math.max(margin, rect.top - popupHeight - 5);
    }

    popup.style.left = Math.min(rect.left, window.innerWidth - 200 - margin) + 'px';
...
// --- LONG PRESS BULK ACTIONS FOR PERIOD MODAL ---
let periodLongPressTimer;

function startPeriodLongPress(event, dateStr) {
    // Only care about primary touch or left click
    if (event.type === 'mousedown' && event.button !== 0) return;

    cancelPeriodLongPress();
    periodLongPressTimer = setTimeout(() => {
        openPeriodBulkActionPopup(event, dateStr);
    }, 600); // 600ms hold time
}

function cancelPeriodLongPress() {
    clearTimeout(periodLongPressTimer);
}

function openPeriodBulkActionPopup(event, dateStr) {
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }
    cancelPeriodLongPress();
    closePeriodPopup(); // Close any existing popups

    const popup = document.createElement('div');
    popup.className = 'period-edit-popup'; // Reuse existing styles
    popup.innerHTML = `
        <div style="font-weight: 600; margin-bottom: 8px; padding-bottom: 5px; border-bottom: 1px solid var(--border-color);">
            Bulk Actions (All Periods)<br>
            <small style="color: var(--medium-text);">${dateStr}</small>
        </div>
        <button style="background: #4CAF50; color: white;" onclick="applyPeriodBulkAction('${dateStr}', 'Attended')">✓ All Attended</button>
        <button style="background: #f44336; color: white;" onclick="applyPeriodBulkAction('${dateStr}', 'Skipped')">✗ All Skipped</button>
        <button style="background: #9e9e9e; color: white;" onclick="applyPeriodBulkAction('${dateStr}', 'Cancelled')">⨯ All Cancelled</button>
        <button style="background: #2196F3; color: white;" onclick="applyPeriodBulkAction('${dateStr}', 'Duty Leave (OD)')">📋 All Duty Leave</button>
        <button style="background: #FF9800; color: white;" onclick="applyPeriodBulkAction('${dateStr}', 'Medical Leave (ML)')">🏥 All Medical Leave</button>
        <button style="background: var(--light-bg); color: var(--dark-text);" onclick="applyPeriodBulkAction('${dateStr}', 'Default')">↺ Reset All (Default)</button>
    `;

    document.body.appendChild(popup);
    popup.style.display = 'block';

    // Prevent clicks inside the popup from triggering the document close listener
    popup.addEventListener('click', function (e) { e.stopPropagation(); });

    // Try to position near click, or fallback to center screen if event coordinates missing
    ...
    let leftPos = Math.min(clientX, window.innerWidth - popupWidth - margin);
    let topPos = clientY + 5;

    // Popup upwards if too close to bottom
    if (topPos + popupHeight > window.innerHeight) {
        topPos = Math.max(margin, clientY - popupHeight - 5);
    }

    popup.style.left = leftPos + 'px';
    popup.style.top = topPos + 'px';

    // Use longer delay to avoid the same click event triggering close
    setTimeout(() => {
        document.addEventListener('click', closePeriodPopup, { once: true });
    }, 300);
}

function applyPeriodBulkAction(dateStr, status) {
    savePeriodViewState();
    closePeriodPopup();
    ...
    // Determine all subjects scheduled for this day
    const date = new Date(dateStr);
    const dayOfWeek = date.getDay();
    const scheduleIndex = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
    ...
    // 1. Check Custom Schedule natively first
    const customSchedule = getCustomScheduleForDate(dateStr);
    if (customSchedule && customSchedule._periods) {
        customSchedule._periods.forEach(code => {
            if (code && !allSubjectCodes.includes(code)) allSubjectCodes.push(code);
        });
    } else if (customSchedule && typeof customSchedule === 'object' && !Array.isArray(customSchedule)) {
        Object.keys(customSchedule).forEach(code => {
            if (code !== '_periods' && customSchedule[code] > 0 && !allSubjectCodes.includes(code)) {
                allSubjectCodes.push(code);
            }
        });
    } else {
        // 2. Default Timetable Schedule
        const periods = getTimetableForDay(scheduleIndex);
        periods.forEach(p => {
            if (p && p.code && !allSubjectCodes.includes(p.code)) {
                allSubjectCodes.push(p.code);
            }
        });

        // Failsafe backup
        if (allSubjectCodes.length === 0 && selectedClass?.subjects) {
            selectedClass.subjects.forEach(subject => {
    ...
    allSubjectCodes.forEach(subCode => {
        ...
        if (status === 'Medical Leave (ML)' && logs[dateStr][subCode] === 'Cancelled') {
            return; // Preserve cancelled classes
        }

        if (status === 'Default') {
            delete logs[dateStr][subCode];
        } else {
            logs[dateStr][subCode] = status;
        }
    });

    // Cleanup specific instances (e.g MATH_p1) to respect bulk application uniformly
    Object.keys(logs[dateStr]).forEach(key => {
        if (key.includes('_p')) {
            if (status === 'Medical Leave (ML)' && logs[dateStr][key] === 'Cancelled') return;
            delete logs[dateStr][key];
        }
    });

    // Refine day deletion: ensure we don't wipe potentially valid legacy logs unless status is explicitly Default
    ...
    // Refresh UI selectively if modal is already open
    const periodModal = document.getElementById('periodAttendanceModal');
    if (periodModal && periodModal.classList.contains('active')) {
        // Atomic update for all cells on this date
        refreshPeriodCells(dateStr, '*', status);
        if (selectedClass?.portalSetup?.active) {
            calculateFromPortal();
        }
    } else {
...
function updatePeriodStatus(dateStr, code, status) {
    ...
    // ML SYNC LOGIC: If setting ML, mark ALL subjects as ML, EXCEPT Cancelled classes
    if (status === 'Medical Leave (ML)') {
        let mlCount = 0;
        allSubjectCodes.forEach(subCode => {
            if (currentDayLog[subCode] !== 'Cancelled') {
                // FIX: CLEAN LEGACY LOGS
                if (logs[dateStr][subCode] && !String(subCode).includes('_p')) {
                    delete logs[dateStr][subCode];
                }
        ...
        // Always ensure the target subject clicked is set correctly (e.g., if it was Cancelled before)
        // FIX: CLEAN LEGACY LOGS
        if (logs[dateStr][code] && !String(code).includes('_p')) delete logs[dateStr][code];
        logs[dateStr][code] = 'Medical Leave (ML)';

        // Clear specific period instances to ensure ML applies globally
        Object.keys(logs[dateStr]).forEach(key => {
            if (key.includes('_p') && logs[dateStr][key] !== 'Cancelled') {
                delete logs[dateStr][key];
            }
        });
        ...
        showToast("Medical Leave applied to applicable subjects.", "success");

        const periodModal = document.getElementById('periodAttendanceModal');
        if (periodModal && periodModal.classList.contains('active')) {
            refreshPeriodCells(dateStr, allSubjectCodes, 'Medical Leave (ML)');
            if (selectedClass?.portalSetup?.active) calculateFromPortal();

            // FIX: Restore scroll explicitly to combat browser layout shift
            requestAnimationFrame(() => {
                const content = periodModal.querySelector('.modal-content') || periodModal;
                if (window.periodModalScrollTop) periodModal.scrollTop = window.periodModalScrollTop;
                if (window.periodModalContentScrollTop) content.scrollTop = window.periodModalContentScrollTop;
            });
        } else {
            refreshPeriodAndHistoryViews();
        }
    ...
    // Check if OTHER subjects are currently ML (meaning this day is largely an ML day)
    const hasOtherML = allSubjectCodes.some(subCode =>
        subCode !== code && currentDayLog[subCode] === 'Medical Leave (ML)'
    );

    // If changing from ML to something else, OR editing an ML day
    if (isCurrentSubjectML || hasOtherML) {
        if (status === 'Cancelled') {
            // Rule: You CAN mix ML and Cancelled seamlessly without destroying other ML blocks
            logs[dateStr][code] = 'Cancelled';
        } else {
            // Rule: If changing to anything else (Attended, Skipped, etc.), abort the entire ML group
            allSubjectCodes.forEach(subCode => {
                if (logs[dateStr][subCode] === 'Medical Leave (ML)') {
                    delete logs[dateStr][subCode]; // Reset the sibling ML classes
                }
            });

            // Apply the requested status only to the clicked cell
            if (status !== 'Default') {
        ...
        const periodModal = document.getElementById('periodAttendanceModal');
        if (periodModal && periodModal.classList.contains('active')) {
            // Because ML siblings might have been deleted, we need to refresh ALL subject cells on this day
            // to ensure they revert to "Default" visually, in addition to the cell we just explicitly changed.
            refreshPeriodCells(dateStr, '*', status); // Pass wildcard so it fixes all siblings
            if (selectedClass?.portalSetup?.active) calculateFromPortal();
        } else {
            refreshPeriodAndHistoryViews();
        }
    ...
    if (status === 'Default') {
        // FIX: CLEAN LEGACY LOGS even when resetting to Default
        const baseCode = code.split('_p')[0];
        if (code.includes('_p') && logs[dateStr][baseCode] && !logs[dateStr][baseCode].includes('_p')) {
            delete logs[dateStr][baseCode];
        }

        delete logs[dateStr][code];

        // Refine day deletion: only delete the date key if it's TRULY empty
        if (Object.keys(logs[dateStr]).length === 0) {
            delete logs[dateStr];
        }
    } else {
        // FIX: CLEAN LEGACY LOGS
        // If we are saving a period-specific log (e.g., MATH_p1), check if a legacy log (MATH) exists and purge it
        const baseCode = code.split('_p')[0];
        if (code.includes('_p') && logs[dateStr][baseCode] && !logs[dateStr][baseCode].includes('_p')) {
            delete logs[dateStr][baseCode];
        }
    ...
    const periodModal = document.getElementById('periodAttendanceModal');
    if (periodModal && periodModal.classList.contains('active')) {
        // If the modal is already open, do an atomic update of just the cells that changed.
        if (status === 'Medical Leave (ML)' || isCurrentSubjectML || hasOtherML) {
            // For complex ML updates, pass all subjects that were just modified
            refreshPeriodCells(dateStr, allSubjectCodes, status);
        } else {
            // For simple single-subject updates, rewrite the exact DOM cell
            refreshPeriodCells(dateStr, code, status);
        }

        // Still need to calculate background stats
        if (selectedClass?.portalSetup?.active) {
            calculateFromPortal();

            // ROLLING BASELINE LOGIC (Sync with saveDailyLog)
            try {
                const logStatus = checkIncompleteLogs();
                if (logStatus.allComplete && typeof currentAnalysisData !== 'undefined' && currentAnalysisData.length > 0) {
                    const updatedBaseline = {};
                    currentAnalysisData.forEach(subject => {
                        updatedBaseline[subject.code] = { attended: subject.attended, total: subject.totalHeld };
                    });
                    const newBaselineDate = formatLocalDate(new Date());
                    selectedClass.portalSetup.baselineDate = newBaselineDate;
                    selectedClass.portalSetup.baselineData = updatedBaseline;
                    ...
                    if (window.SyncManager) SyncManager.uploadAll();
                    if (typeof showToast === 'function') {
                        showToast('Baseline Updated', `Portal baseline moved to ${newBaselineDate}.`, { icon: '📅', duration: 4000 });
                    }
                }
            } catch (e) { console.error('Auto-baseline update failed (period mode)', e); }
        }
...
// Atomic DOM updater for cells to prevent full screen re-render and scroll jump
function refreshPeriodCells(dateStr, codeOrArray, fallbackStatus) {
    const periodModal = document.getElementById('periodAttendanceModal');
    if (!periodModal) return;
    ...
    if (codeOrArray === '*') {
        // Wildcard: update all cells for this date
        targets = Array.from(periodModal.querySelectorAll(`.period-cell[data-date="${dateStr}"]`));
        // Filter out holiday/free periods that don't have a code
        targets = targets.filter(c => c.hasAttribute('data-code'));
    } else {
        const codes = Array.isArray(codeOrArray) ? codeOrArray : [codeOrArray];

        codes.forEach(code => {
            // Try exact match first (e.g., MATH_p1)
            const exactCells = periodModal.querySelectorAll(`.period-cell[data-date="${dateStr}"][data-code="${code}"]`);

            if (exactCells.length > 0) {
                targets.push(...Array.from(exactCells));
            } else {
                // Try base code match (e.g., MATH)
                const allDateCells = periodModal.querySelectorAll(`.period-cell[data-date="${dateStr}"]`);
                const baseCode = code.split('_p')[0];
                const matchedBase = Array.from(allDateCells).filter(c => {
                    const cellCode = c.getAttribute('data-code');
                    return cellCode && cellCode.startsWith(baseCode);
                });
                targets.push(...matchedBase);
            }
        });
    }

    targets.forEach(cell => {
        const cellCode = cell.getAttribute('data-code');

        // 1. Resolve actual status from database to prevent blind overwrites
        let actualStatus = dayLog[cellCode];
        if (!actualStatus && cellCode) {
            const baseCode = cellCode.split('_p')[0];
            actualStatus = dayLog[baseCode];
        }
        ...
        if (!actualStatus || actualStatus === 'Default') {
            cellClass = 'not-logged';
            symbol = '?';
        }
        else if (actualStatus === 'Attended') { cellClass = 'present'; symbol = 'P'; }
        else if (actualStatus === 'Skipped') { cellClass = 'absent'; symbol = 'A'; }
        else if (actualStatus === 'Cancelled') { cellClass = 'cancelled'; symbol = 'C'; }
        else if (actualStatus === 'Duty Leave (OD)') { cellClass = 'od'; symbol = 'OD'; }
        else if (actualStatus === 'Medical Leave (ML)' || String(actualStatus).toUpperCase() === 'ML') {
            cellClass = 'ml';
            symbol = 'ML';
            warningTitle = '✓ Usable ML';
        }
        ...
        // Update Title (Hover text)
        const cellTitle = cell.getAttribute('title') || '';
        const baseTitle = cellTitle.split(' | ')[0]; // Extract subject name
        cell.setAttribute('title', `${baseTitle} | ${actualStatus} ${warningTitle}`.trim());

        // Handle Glow Effects depending on status
        if (cellClass === 'not-logged') {
            cell.style.boxShadow = '';
            cell.style.background = '';
            cell.style.color = '';
        } else if (cellClass === 'ml') {
            cell.style.boxShadow = '0 0 8px 2px #FF980060, inset 0 0 0 2px #FF9800';
            cell.style.color = '#FF9800';
            cell.style.background = 'rgba(255, 152, 0, 0.1)';
        } else if (cellClass.includes('ml-invalid')) {
            cell.style.boxShadow = '0 0 10px 3px rgba(255, 59, 48, 0.4), inset 0 0 0 2px rgba(255, 59, 48, 1)';
            cell.style.color = 'white';
            cell.style.background = '#ff3b30';
        } else {
            // For regular statuses, reset inline styles so the external CSS reigns supreme,
            // except if there's a subject-specific glow which technically requires a full re-render
            // to get perfect, but clearing it defaults to standard CSS colors which is fine atomically.
            cell.style.boxShadow = '';
            cell.style.background = '';
            cell.style.color = '';
        }
...
function refreshPeriodAndHistoryViews(forcePeriodFullRender = true) {
    // Refresh the period modal
    if (forcePeriodFullRender) {
        openPeriodAttendanceModal();
    }
